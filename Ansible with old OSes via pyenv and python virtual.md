...
First, you must be completely setup with pyenv, have an appropriate version of Python installed and active, and an appropriate version of Ansible installed to your virtual environment. Does sound familiar to you? If not then please review my document on my Git Sparkyz Notez site.  
...
**My current setup has a few different combinations of Python and Ansible. Yours may too. If you have more than one virutal environment then this next step is important.**
...
Both of those are antiques but I have a few sad, obsolete Red Hat 6 (RH6) servers that must be managed. This is the setup that still works for that RH6 environent.
...
At this point you will need your inventory file and for the inventory location to be specified in your ansible.cfg file. I dive into that here because I use a few non-standard naming conventions. I use INI format and I name my file *inventory.ini* because I feel like there are already too many Linux/Unix files named *hosts*.
...
It would be a poor practice to make this permanent. Just set it, add your hosts, then restart your shell to clear it.

**I am using Ansible 2.12 for the initial tests because I have obsolete systems. Red Hat 6 and 7 both use Python 2.7 by default. Red Hat 7 can install Python 3 from repositories. Not so for Red Hat 6. I'll switch to Ansible 2.21 later on in this process so we can enjoy a new set of errors to work through.**
...
- `--user root` I'm not logged into my control node as root but I don't have any accounts on my guest hosts yet. I'm connecting as root
...
I allowed RSA only in my ansible control node's user account. Add the following to your `~/.ssh/config` file. (Be sure your permissions are 600, read-write only for the user.) Also, add this to the bottom of your config file so that any global settings aren't overrided. Limit the scope as much as you can. Here's my fix in my file for my lab 172.17.1.0/24 network.  
```
Host 172.17.1.*
...
Some systems may throw an error about not being able to find Python. In my experience this is rare on Linux systems. Not so for FreeBSD Unix. It happens on versions 14 and 15, and probably older versions as well. FreeBSD doesn't install Python by default. I did prep the systems by installing Python, but it doesn't install to the typical location that Linux would use. Instead it goes to a more Unix typical location of `/usr/local/bin/python3`.
...
#### Warnings Galore!
Newer versions of Ansible throw tons of warnings. Some warnings are just an annoyance. In my opinion, if a warning is thrown on every managed host, that warning is not very useful.
...
Now it's time to restart your shell to clear any environment variable tweaks you enabled. Ping everything one more time. Still all Green and OK? Good. You're now ready to deploy a limited permissions Ansible access user to all your nodes and add them to your sudo files. We never want to user root accounts again, but that's for a different set of instructions. I'll share my basic ansible user deployment playbook and approach on my Git Sparky'z Notez site.
